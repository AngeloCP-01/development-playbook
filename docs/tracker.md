...
the defective one, and it only fires on a **cold** dev server — Fast Refresh rebuilds a
module without re-running creation-time key validation, so an edit-and-reload reads clean
