...
- ~~**`W-5` (deploy)**~~ — **live 2026-08-11** at `https://acp-dev-playbook.vercel.app`. What
  remains under this heading is **post-deployment verification per `docs/14`**, which is now
  unblocked for the first time: the audit suite assumes `:3100`, so retargeting it at the
  deployed URL is its own slice — and it is the one that turns 14 local checks into a real
