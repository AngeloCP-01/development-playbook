...
|---|---|---|---|---|
| 2026-08-24 | TD-43 | **The missing key that was never missing.** React validates keys twice and unwraps lazy nodes to different depths: `validateChildKeys` stamps the static-children exemption one level down, `warnOnInvalidKey` checks all the way down. `Architecture` is a server component, so every step's `content` crosses the RSC boundary as a lazy chunk, and the last one the server flushes arrives wrapped twice — never stamped, then reported as an unkeyed list child. The panel now renders `<>{step.content}</>`, keeping the streamed node out of a multi-child array so the single-child path runs. **The previous session's two recorded discriminators are replaced, not refined**: it follows the last content *flushed* rather than the last *index*, and the id `traps` was never the discriminator | One-line production change in `web/src/components/Stepper.tsx`. **RED** with the pin removed (1 failed at `/stages/03-architecture#traps`), **GREEN** with the fragment (1 passed, 42.4s over 76 URLs), **teeth check** reverting *only* `<>{step.content}</>` and leaving the comment (1 failed, same URL, same message), then restored. Prerendered HTML for the stage is **byte-identical** before and after — 284,405 bytes both, empty diff once the build id and one chunk hash are normalised — so the fragment changes no DOM. Gate: lint 0, typecheck 0, **660/82**, build clean, audit **18/18**, `test:dev-console` **1/1 unpinned**. Reproduction: `docs/verification/td-43-lazy-key-warning.md`. **D-86's pin retired itself as designed** — removing it is what produced the failing test. One new decision, **D-87** | **Deferred:** no whole-branch or per-task reviewer ran, same as the round before it — the branch is one production line plus records, but every review this project has run has found something. The upstream asymmetry in React is left unreported. The three LCP `loading="eager"` hints the dev sweep still prints as unmatched noise on `/reference` sheets. TD-31 and TD-33, untouched |
...
### ~~TD-43~~ — A missing key at `/stages/03-architecture#traps` · **CLOSED 2026-08-24**

**Closed by a fragment in `Stepper`, and there was never a missing key.** The full
...
crosses the RSC boundary as a lazy chunk — and the last content the server flushes arrives
wrapped **twice**, because its own children are still pending when it resolves. Fulfilled,
but with another lazy inside it, it satisfies neither branch of the first function and is
never stamped; the second walks all the way down to an element with `key === null` and
reports it. The panel now renders `<>{step.content}</>`, which keeps the streamed node out
of a multi-child array so the single-child path runs and the array check never applies.

**Two of the previous investigation's conclusions are replaced rather than refined.** It
does not follow the last *index*; it follows the last content *flushed*, which is the same
step only by coincidence. And the discriminator was never a step whose id is `traps` —
removing that step changed which content was flushed last. Both recorded results were true
observations with the wrong cause attached, which is why eighteen probes could not close it:
they were all searching for an array with a missing key, and no such array exists.
...
URL. The prerendered HTML for the stage is byte-identical before and after — 284,405 bytes
both, and normalising the build id and one chunk hash leaves an empty diff — so the fragment
changes no DOM. Gate on the branch: lint 0, typecheck 0, **660/82**, build clean, audit
