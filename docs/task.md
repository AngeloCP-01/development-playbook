...
### W-3.1b — Stage 03 completeness: resilience, consistency, evolution ☐ *(after W-3.2)*
...
- [ ] **Resilience patterns** — timeout, retry with exponential backoff and jitter, circuit
...
- [ ] **Consistency and concurrency** — CAP named, eventual consistency as a term rather than
...
- [ ] **Safe schema evolution** — **expand-contract / parallel change**, and **strangler fig**
...
- [ ] **Statelessness and scaling mechanics** — statelessness (which is what *makes* the
...
- [ ] **Fitness functions** — evolutionary architecture's idea that a characteristic should be
...
- [ ] **Widen the characteristics trace table** past three rows, so the ten-item candidate list
      stops promising more than the stage delivers
- [ ] Expect **one new `###` section** (schema evolution); the rest extend existing sections.
...
- [ ] **Port this round's content into the app as part of this round**, not as a third pass.
...
- [ ] Glossary terms for every new concept (`terms.ts` → `pnpm gen:glossary`), and **grep
      `terms.ts` before writing prose** per D-47
- [ ] Cold-reader re-run on the amended doc, same shift-swap product; **budget a fix wave
      after it and verify the wave itself** (D-48)
- [ ] `humanizer:humanizer` pass (D-20); consultability check, which the cold reader cannot do
