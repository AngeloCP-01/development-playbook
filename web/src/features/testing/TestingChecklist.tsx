 ...
 * "## Scaling to a team".
 *
 * Modelled on stage 04's `SetupChecklist` and stage 05's `DevChecklist` —
 * same persisted-worksheet shape, same storage mechanism, its own key.
 * Progress is keyed on `DoneItem.id` rather than position, which is why
 * `checklist.ts` gives its items slugs: reordering the doc's checkboxes must
 * not silently reset a reader's ticks.
 *
 * `ARTIFACT_LIST` renders as a plain list above the checklist, same as
 * stage 05's `ARTIFACT_ITEMS` — it names what the seven checkboxes below are
 * checking, not a second tickable exercise.
 *
 * State comes from `useLocalStorage`, which reads through
 * `useSyncExternalStore` rather than `useEffect` + `setState` —
 * `react-hooks/set-state-in-effect` is an error in this codebase, not a
 * warning.
 *
...
export function TestingChecklist() {
  const {
    ...
    setValue,
    reset,
  } = useLocalStorage<string[]>(TESTING_CHECKLIST_KEY, NOTHING_TICKED)
  const idBase = useId()
      ...
      <Card className="p-0">
        ...
        <div className="flex flex-wrap items-center gap-3 border-t border-line px-5 py-3.5">
          ...
          <p className="text-sm text-subtle" aria-live="polite">
            {complete
              ? 'Every box ticked — the suite is done, and you proved it rather than assumed it.'
