 ...
 * done", plus a two-note `TEAM` disclosure.
 *
 * Same shape as stage 06's `TestingChecklist`: one `Card` holding an
 * artifacts list, a persisted done checklist keyed on `DoneItem.id`
 * (position-independent, so reordering the doc's checkboxes cannot
 * silently reset a reader's ticks), and a `TeamNotes` disclosure below it.
 * State goes through `useLocalStorage`, which reads via
 * `useSyncExternalStore` rather than `useEffect` + `setState` —
 * `react-hooks/set-state-in-effect` is an error here, not a warning.
 *
 * The progress indicator renders "N of M" rather than "N/M" so it reads as
 * a sentence fragment next to the heading, and it stays silent at zero
 * rather than announcing "0 of 6" on first paint.
 *
 * Checked means *done*, so the control is tinted `go`, not `brand` —
 * `brand` is attention, not approval.
 *
 * `preview`'s label keeps the doc's `(12)` as backticked plain text
 * (`InlineCode` renders it verbatim); `getStage('12-staging')` supplies a
 * real `next/link` alongside it instead, same device stage 06 uses for its
 * own cross-stage note.
 */

export const CODE_REVIEW_CHECKLIST_KEY = 'code-review-checklist'
...
export function CodeReviewChecklist() {
  const {
    ...
    setValue,
    reset,
  } = useLocalStorage<string[]>(CODE_REVIEW_CHECKLIST_KEY, NOTHING_TICKED)
  const idBase = useId()

  const count = DONE.filter((item) => ticked.includes(item.id)).length
  const complete = count === DONE.length
  const previewStage = getStage('12-staging')
      ...
      <Card className="p-0">
              ...
              <li key={item.id}>
                ...
                >
                  ...
                  >
                    ...
                    {item.id === 'preview' && previewStage && (
                      <>
                        {' '}
                        <Link
                          href={`/stages/${previewStage.slug}`}
                          className="t-label border border-line px-1.5 py-0.5 text-brand transition-colors duration-150 hover:border-brand"
                        >
                          {previewStage.title}
                        </Link>
                      </>
        ...
        <div className="flex flex-wrap items-center gap-3 border-t border-line px-5 py-3.5">
          ...
          <p className="text-sm text-subtle" aria-live="polite">
            {complete
              ? 'Every box ticked — the diff is ready to merge, not just written.'
