 ...
 * the syntax is not.
 *
 * `html` is looked up from the generated table (`pnpm gen:highlighted`), not
 * computed here — see `generate-highlighted.ts` for why highlighting at
 * import time does not survive contact with every consumer of this registry.
 * A code string not yet in that table renders as plain text rather than
 * throwing, which is what makes `pnpm gen:highlighted` a thing to remember to
 * run rather than a thing that blocks every other change.
 */
