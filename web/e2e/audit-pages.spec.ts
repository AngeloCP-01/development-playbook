...
test('refuses to sweep a stage that is ready but renders no steps, since silence there is the failure TD-12 describes', async ({
  ...
  //
  // This used to name `04-project-setup`, then `05-development` — each time,
  // porting that stage flipped `ready` and gave it a full rail, turning a
  // stage that must render nothing into one that renders steps, so the test
  // went red having found no defect and got moved rather than deleted. It
  // names `06-testing` for no reason beyond it being the next stage that is
  // not built; when 06 is ported, move this again rather than deleting it.
  await expect(readStepIds(page, '06-testing')).rejects.toThrow(
    /renders no steps/,
  )
})
